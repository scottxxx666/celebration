...
// (216px = 2x logical) so every sprite scales consistently in
...
// only from `hh` here. And since wall sections spawn four obstacles per beat, a
// wide `hw` (zombie 47 vs the authored 25) makes walls block a row for longer;
...
export const OBSTACLE_SPRITES = [
  {
    ...
    hh: 54, // logical half-height at front-row scale; drives the uniform
            // scale applied to the sprite (width follows the image aspect)
    hw: 27, // collision AABB half-width AND spawn-timing distance — tuned
